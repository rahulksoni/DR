...
prompt = 'Enter 0 for V or Zn, otherwise 1 for V-Zn combined run or 2 for Input Output Balancing? ';
singleOrBoth = input(prompt);
...
elseif (singleOrBoth == 1)
    ...
    [c,ceq_x]   = DR_SQP_Constraints_VandZn(x); 
elseif (singleOrBoth == 2)
    [c,ceq_x0]  = DR_SQP_Constraints_InputOutputBalancing(x0);
    [c,ceq_x]   = DR_SQP_Constraints_InputOutputBalancing(x); 
end
...
x_write        = xlswrite(xlfilename,x',SQPSheetName,x_writerange)';
